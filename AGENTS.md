...
- **面板只是管理面板，不是运行时依赖**（核心原则）：除监控采集外（需面板自身的后台调度器驱动周期采样），所有功能都必须是 FreeBSD 原生已有的能力。面板只做便捷的可视化管理，绝不引入"离开面板就无法运行"的依赖。关掉面板后，由面板配置的一切（rc.conf、sysctl、ZFS、Jail、Bhyve、SMB 等）照常工作。开发新功能时必须遵守：操作目标始终是 FreeBSD 自带的或可独立安装运行的服务/工具，面板通过调用或配置它们来实现管理，而非自己实现运行时逻辑。

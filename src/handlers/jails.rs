...
const VNET_HELPER_VERSION: u32 = 20;
...
# Usage:
#   fwp-vnet up   <jailname> <epair_id> <bridge>
#   fwp-vnet down <jailname> <epair_id>
#   fwp-vnet init <jailname> <epair_id> dhcp
#   fwp-vnet init <jailname> <epair_id> static <ip> <gateway>

ACTION="$1"
NAME="$2"
EPAIR_ID="$3"
HOST_IF="epair${EPAIR_ID}a"
JAIL_IF="epair${EPAIR_ID}b"

case "$ACTION" in
up)
    BRIDGE="$4"
    /sbin/ifconfig "$HOST_IF" destroy 2>/dev/null
    /sbin/ifconfig "epair${EPAIR_ID}" create up || exit 1
    if [ -n "$BRIDGE" ]; then
        /sbin/ifconfig "$BRIDGE" addm "$HOST_IF" 2>/dev/null
    fi
    ;;
down)
    /sbin/ifconfig "$HOST_IF" destroy 2>/dev/null
...
init)
    ...
    case "$MODE" in
    dhcp)
        /usr/sbin/jexec "$NAME" /sbin/dhclient "$JAIL_IF"
        ;;
    static)
        IP="$5"
        GW="$6"
        [ -n "$IP" ] && /usr/sbin/jexec "$NAME" /sbin/ifconfig "$JAIL_IF" inet "$IP"
...
fn generate_jail_block_from_params(
    ...
    if vnet_auto {
        ...
        lines.push(format!("    vnet.interface = \"epair{epair_id}b\";"));
        ...
        match ip4 {
            ...
            "dhcp" => {
                ...
                lines.push(format!("    exec.poststart += \"{VNET_HELPER} init ${{name}} {epair_id} dhcp\";"));
            }
            _ => {
